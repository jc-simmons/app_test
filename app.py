...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        names='Launch Site', 
        title='Total Success Percentage per Launch Site')
        return fig
    else:
...
def get_scatter_plot(slider_range,entered_site):
   ...
   # filtered_df = spacex_df[spacex_df['Launch Site']==str(entered_site)]
    ...
    if entered_site == 'ALL': 
        filtered_df=spacex_df.loc[(spacex_df['Payload Mass (kg)'] > floor) & (spacex_df['Payload Mass (kg)'] < ceil)]
        fig = px.scatter(
        filtered_df, x="Payload Mass (kg)", y="class", 
        color="Booster Version Category",
        title='Correlation between Payload and Success for all State')
        return fig
    else:
        filtered_df=spacex_df.loc[(spacex_df['Payload Mass (kg)'] > floor) & (spacex_df['Payload Mass (kg)'] < ceil)]
